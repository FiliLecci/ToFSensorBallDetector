...
long get_millis(struct timespec ts)
{
...
void calcola_distanza(float angle, Lidar *lidar, Sphere s) {
    ...
    // Gestione esplicita dei casi in cui l'angolo è 0°, 90°, 180° o 270°
    ...
    if (vertical_line) {
        ...
        // Gestione del caso di linea orizzontale
        a = 1;
        b = -2 * xc;
        c = xc * xc + (yl - yc) * (yl - yc) - R * R;
    } else {
    ...
    if (vertical_line) {
        ...
        y1 = yl;
        y2 = yl;
        x1 = xc + sqrt(discriminante) / (2 * a);
        x2 = xc - sqrt(discriminante) / (2 * a);
    } else {
...
void draw_scene(SDL_Renderer *renderer, Lidar *lidar, Sphere *s, CoordList posizioni[])
{
    ...
    if(posCounter == 50)
    {
        ...
        long start_ms = get_millis(last_checkpoint_ts);
        long end_ms = get_millis(last_ts);
